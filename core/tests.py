...
from coursefinder.models import CourseFinderLandingPage, CourseFinderChooseCountry


class CoreUtilsTests(UniSimpleTestCase):

    def test_get_page_for_language_returns_english_page_if_it_exists(self):
        created_page = PageFactory.create_course_finder_landing_page('English course finder')
        self.assertIsNotNone(created_page)

        found_page = get_page_for_language(enums.languages.ENGLISH, CourseFinderLandingPage.objects.all())
    ...
    def test_get_page_for_language_returns_english_page_if_multiple_english_pages_exist(self):
        created_page = PageFactory.create_course_finder_landing_page('English course finder')
        second_created_page = PageFactory.create_course_finder_landing_page('English course finder 2')
        self.assertIsNotNone(created_page)
    ...
    def test_get_page_for_language_returns_no_page_for_english_if_no_english_exists(self):
        pages = CourseFinderLandingPage.objects.all()
    ...
    def test_get_page_for_language_returns_welsh_page_if_it_exists(self):
        welsh_root = PageFactory.create_course_finder_landing_page('cy')
...
class CoreModelsTests(UniSimpleTestCase):
    ...
    def test_get_language_returns_en_for_english_page(self):
        english_root = PageFactory.create_course_finder_landing_page('home')
