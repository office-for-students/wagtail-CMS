...
def courses_detail(request, institution_id, course_id, kis_mode, language=enums.languages.ENGLISH):
    ...
    print_salary_data_to_file = True

    if print_salary_data_to_file:
        course_print_file = open("course_print_file.json", "w")
        for agg in course.salary_aggregates:
            course_print_file.write('================================================\n')
            course_print_file.write('SALARY_AGGREGATE RECORD:\n')
            course_print_file.write('================================================\n\n')

            for inst in agg.aggregated_salaries_inst:
                unavail = ""
                med = ""
                lq = ""
                uq = ""
                ...
                if 'reason' in inst.display_unavailable_info(): unavail = inst.display_unavailable_info()['reason']
                if hasattr(inst, 'med'): med = inst.med
                if hasattr(inst, 'lq'): lq = inst.lq
                if hasattr(inst, 'uq'): uq = inst.uq
                if hasattr(inst, 'pop'): pop = inst.pop
                if hasattr(inst, 'resp_rate'): resp = inst.resp_rate

                course_print_file.write('**SALARY_INST RECORD:**\n')
                course_print_file.write('"unavail":' + repr(unavail) + '\n')
                course_print_file.write('"med":' + med + '\n')
                course_print_file.write('"lq":' + lq + '\n')
                course_print_file.write('"uq":' + uq + '\n')
                course_print_file.write('"pop":' + pop + '\n')
                course_print_file.write('"resp_rate":' + resp + '\n')
                course_print_file.write('\n')
                #course_print_file.write(json.dumps(json.loads(course.go_salaries_inst), indent=4, sort_keys=True))

            for sect in agg.aggregated_salaries_sector:
                unavailable_region_not_exists = ""
                no_salary_node = ""
                med_uk = ""
                lq_uk = ""
                ...
                if 'unavailable_region_not_exists' in sect.display_unavailable_info(): unavailable_region_not_exists = sect.display_unavailable_info()['unavailable_region_not_exists']
                if hasattr(sect, 'med_uk'): med_uk = sect.med_uk
                if hasattr(sect, 'lq_uk'): lq_uk = sect.lq_uk
                if hasattr(sect, 'uq_uk'): uq_uk = sect.uq_uk
                if hasattr(sect, 'pop_uk'): pop_uk = sect.pop_uk
                if hasattr(sect, 'resp_uk'): resp_uk = sect.resp_uk

                course_print_file.write('**SALARY_SECT RECORD:**\n')
                course_print_file.write('"unavailable_region_not_exists":' + repr(unavailable_region_not_exists) + '\n')
                course_print_file.write('"no_salary_node":' + no_salary_node + '\n')
                course_print_file.write('"med_uk":' + med_uk + '\n')
                course_print_file.write('"lq_uk":' + lq_uk + '\n')
                course_print_file.write('"uq_uk":' + uq_uk + '\n')
                course_print_file.write('"pop_uk":' + pop_uk + '\n')
                course_print_file.write('"resp_uk":' + resp_uk + '\n')
                course_print_file.write('\n')
              
        course_print_file.close()
