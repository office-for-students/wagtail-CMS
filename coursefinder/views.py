...
def results(request, language=enums.languages.ENGLISH):
    ...
    comparison_page = get_page_for_language(
        language, CourseComparisonPage.objects.all()
    )
...
def course_finder_results(request, language=enums.languages.ENGLISH):
    ...
    if "distance" in filters and "campus" not in filters:
        ...
        countries_query = (
            ",".join(query_params.getlist("countries_query"))
            if "countries_query" in query_params
            else None
        )

    institution_query = (
        "@".join(query_params.getlist("institution_query"))
        if "institution_query" in query_params
        else None
    )
    ...
    distance_query = (
        query_params.get("distance") if "distance" in query_params else None
    )
    postcode_query = (
        (postcode + "," + distance_query) if postcode and distance_query else {}
    )
    ...
    course_finder_search = CourseFinderSearch(
        query_params.get("subject_query", None),
        institution_query,
        countries_query,
        postcode_query,
        filters,
    ...
    context.update(
        {
            ...
            "search_info": {
                ...
                "number_options_selected": translations.term_for_key(
                    "number_options_selected", language
                ),
                "institution_name": translations.term_for_key(
                    "institution_name", language
                ),
                "select_all_results": translations.term_for_key(
                    "select_all_results", language
                ),
                "select_all_institutions": translations.term_for_key(
                    "select_all_institutions", language
                ),
            },
        }
    )
